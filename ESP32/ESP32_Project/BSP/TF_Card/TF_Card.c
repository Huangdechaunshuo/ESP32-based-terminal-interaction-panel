...
void TF_Card_Read_audio(const char *path)
{
    ...
    // MP3解码输出的16bit立体声PCM缓冲区
    char pcm_buf[EXAMPLE_MAX_CHAR_SIZE];  // 定义一个字符串数组
    
    FILE *f = fopen(path, "r");  // 以只读方式打开文件

    while(!feof(f))
    {
        memset(pcm_buf, 0, sizeof(pcm_buf));  // 清空字符串数组
        size_t len = fread(pcm_buf, sizeof(char), sizeof(pcm_buf), f);

        // 将解码后的PCM音频通过I2S发送给MAX98357功放
        // size_t bytes_written;
        i2s_write_audio_data(pcm_buf, sizeof(pcm_buf), &len, portMAX_DELAY);
    }
...
void TF_Card_Init(void)
{
    ...
    // 新建一个hello.txt文件，并且给文件中写入几个字符
    const char *file_hello = MOUNT_POINT"/你好hello.txt";
    char data[EXAMPLE_MAX_CHAR_SIZE];
    snprintf(data, EXAMPLE_MAX_CHAR_SIZE, "%s %s!\n", "你好hello", card->cid.name);
    ret = s_example_write_file(file_hello, data);
    if (ret != ESP_OK) {
        return;
    }

    const char *I2S_FILE = MOUNT_POINT"/音频/晴天.wav";

    // 打开hello.txt文件，并读出文件中的内容
    TF_Card_Read_audio(I2S_FILE);
